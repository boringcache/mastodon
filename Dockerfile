...
# BoringCache supplies session-only remote storage settings when tool caching is enabled.
...
# Native gem extensions use the same compiler cache as the media stages.
...
RUN \
  ...
  bundle clean --force; \
  ccache --show-stats; \
  # Cache mounts are not committed to image layers, so preserve the installed bundle
  mkdir -p /opt/bundle; \
